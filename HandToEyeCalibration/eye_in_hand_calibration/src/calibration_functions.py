...
def take_photos():
    ...
    print([(p.split("/")[-1].split("_")[0]) for p in images])
    
    order = np.argsort([(p.split("/")[-1].split("_")[0]) for p in images])
    print(order)
    images = images[order]
    print(images)

    im = PIL.Image.open(images[0])
    #im = PIL.Image.open("/home/ictg-2/eye_in_hand_calibration/images/example.png")
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
    plt.imshow(im)
    plt.show()
...
def aniket_invert_transformation(R_a2b, t_a2b):
    ...
    for i in range(len(R_a2b)):
      f = PyKDL.Frame(PyKDL.Rotation.RPY(R_a2b[i][0], R_a2b[i][1], R_a2b[i][2]),  PyKDL.Vector(t_a2b[i][0], t_a2b[i][1], t_a2b[i][2])).Inverse()
      t = f.p
      r = f.M.GetRPY()
      R_b2a.append([r[0], r[1], r[2]])
      t_b2a.append([t[0], t[1], t[2]])
...
def read_image_transforms(txtfile):
    ...
    os.chdir("..")
    image_dir = os.path.join("/home/niklaus/aniketCodes/aaFinal_ArucoDetection/eye_in_hand_calibration", txtfile)
